...
final class AeroDataBoxService {
    ...
    func fetchFlightData(flightNumber: String, localDepartureDate: String) async -> [FlightAwareData] {
        ...
        guard let apiDate = convertDateToAPIFormat(localDepartureDate) else {
            print("🌐 AeroDataBox: Failed to convert date '\(localDepartureDate)' — expected dd/MM/yyyy")
            return []
        }
    ...
    private func fetchForAPIDate(_ apiDate: String, flightNumber: String) async -> [FlightAwareData] {
        ...
        guard let url = URL(string: urlString) else {
            print("🌐 AeroDataBox: Could not build URL for date \(apiDate)")
            return []
        }

        print("🌐 AeroDataBox: GET \(urlString)")
        ...
        do {
            ...
            print("🌐 AeroDataBox (\(apiDate)): Network error — \(error.localizedDescription)")
            return []
        }

        if let http = response as? HTTPURLResponse {
            print("🌐 AeroDataBox (\(apiDate)): HTTP \(http.statusCode)")
            guard http.statusCode == 200 else {
                if http.statusCode != 404 {
                    let body = String(data: data, encoding: .utf8) ?? "<no body>"
                    print("🌐 AeroDataBox (\(apiDate)): HTTP \(http.statusCode) — \(body.prefix(200))")
                }
                return []
            }
        }
        ...
        if let rawJSON = String(data: data, encoding: .utf8) {
            print("🌐 AeroDataBox (\(apiDate)): Raw JSON (first 1500 chars): \(rawJSON.prefix(1500))")
        }
        ...
        do {
            ...
            print("🌐 AeroDataBox (\(apiDate)): JSON decode failed — \(error)")
            return []
        }

        print("🌐 AeroDataBox (\(apiDate)): Decoded \(flights.count) flight object(s)")
    ...
    private func parseFlights(_ flights: [ADBFlight], searchDate: String) -> [FlightAwareData] {
        ...
        for (idx, flight) in flights.enumerated() {
            ...
            print("🌐 AeroDataBox (\(searchDate)) [\(idx)] number=\(flightNum), status=\(status), isCargo=\(isCargo), aircraftReg=\(aircraftReg ?? "nil")")

            if isCargo {
                print("🌐 AeroDataBox (\(searchDate)) [\(idx)]: Skipping cargo flight")
                continue
            }
            ...
            guard let depICAO = flight.departure.airport.icao, !depICAO.isEmpty,
                ...
                print("🌐 AeroDataBox (\(searchDate)) [\(idx)]: Missing ICAO code(s) — skipping")
                continue
            }
            ...
            print("🌐 AeroDataBox (\(searchDate)) [\(idx)] DEP \(depICAO):")
            print("   scheduledTime (STD) = \(depScheduled ?? "nil")")
            print("   revisedTime  (OUT)  = \(depRevised  ?? "nil")")
            print("   runwayTime  (T/O)   = \(depRunway   ?? "nil")")
            ...
            print("🌐 AeroDataBox (\(searchDate)) [\(idx)] ARR \(arrICAO):")
            print("   scheduledTime (STA) = \(arrScheduled ?? "nil")")
            print("   revisedTime  (IN)   = \(arrRevised   ?? "nil")")
            print("   runwayTime  (LDG)   = \(arrRunway    ?? "nil")")
            ...
            guard let depRaw = depRevised ?? depScheduled,
                ...
                print("🌐 AeroDataBox (\(searchDate)) [\(idx)] \(depICAO)→\(arrICAO): No usable times — skipping")
                continue
            }

            guard let depTime = parseUTCTime(depRaw),
                  let arrTime = parseUTCTime(arrRaw) else {
                print("🌐 AeroDataBox (\(searchDate)) [\(idx)] \(depICAO)→\(arrICAO): Failed to parse times '\(depRaw)' / '\(arrRaw)' — skipping")
                continue
            }
            ...
            print("🌐 AeroDataBox (\(searchDate)) [\(idx)] \(depICAO)→\(arrICAO) RESOLVED:")
            print("   OUT (revisedTime) = \(depTime) UTC (actual=\(departureIsActual))")
            print("   IN  (revisedTime) = \(arrTime) UTC (actual=\(arrivalIsActual))")
            print("   STD               = \(scheduledDepTime ?? "nil") UTC")
            print("   STA               = \(scheduledArrTime ?? "nil") UTC")
            print("   T/O (runwayTime)  = \(runwayDepTime ?? "nil") UTC")
            print("   LDG (runwayTime)  = \(runwayArrTime ?? "nil") UTC")
            print("   Date              = \(flightDate)")
