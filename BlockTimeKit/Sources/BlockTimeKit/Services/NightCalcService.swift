...
class NightCalcService: @unchecked Sendable {
    ...
    nonisolated func calculateNightTime(
        ...
        guard let fromCoords = airportLookup.getCoordinates(for: fromAirport.uppercased()),
            ...
            print("[NightCalc] Could not find coordinates for airports \(fromAirport) or \(toAirport)")
            return nil
        }
...
private class AirportLookup: @unchecked Sendable {
    ...
    private func loadAirports() {
        guard let path = Bundle.main.path(forResource: "airports.dat", ofType: "txt") else {
            print("Could not find airports.dat.txt in bundle")
            return
        }
        ...
        do {
            ...
            print("Error reading airports.dat.txt: \(error)")
        }
    }
    ...
    nonisolated func getCoordinates(for icao: String) -> (latitude: Double, longitude: Double)? {
        return airports[icao.uppercased()]
    }
}

// MARK: - Supporting Functions (moved into NightCalcService extension for nonisolated access)
extension NightCalcService {
    private nonisolated func parseUTCString(_ timeStr: String, on date: Date = Date()) -> Date? {
    ...
    private nonisolated func nightPortion(
        ...
        guard let depDate = parseUTCString(departureUTC, on: flightDate) else {
            print("[NightCalc] Invalid departure time format: '\(departureUTC)'")
            return 0
        }
        print("[NightCalc] nightPortion: departureUTC='\(departureUTC)', flightDate=\(flightDate), depDate=\(depDate), flightDurationHours=\(flightDurationHours), segments=\(segments)")
        ...
        print("[NightCalc] nightPortion result: \(nightSegmentCount) night segments out of \(segments) total = \(nightHours) hours")
        return nightHours
    }

    private nonisolated func isNightInternal(at lat: Double, lon: Double, time: Date) -> Bool {
        ...
        let gst = greenwichSiderealTime(for: time.timeIntervalSince1970)
        let lst = gst + lon*rad
    ...
    private nonisolated func julianDay(for date: Date) -> Double {
        var cal = Calendar(identifier: .gregorian)
        cal.timeZone = TimeZone(secondsFromGMT: 0)!
        ...
        let D = Double(comps.day!)
            + (Double(comps.hour!) + Double(comps.minute!)/60 + Double(comps.second!)/3600) / 24.0
        if M <= 2 { Y -= 1; M += 12 }
    ...
    private nonisolated func greenwichSiderealTime(for unixTime: TimeInterval) -> Double {
