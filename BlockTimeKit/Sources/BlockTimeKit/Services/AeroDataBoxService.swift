...
final class AeroDataBoxService {
    ...
    private func parseFlights(_ flights: [ADBFlight], searchDate: String) -> [FlightAwareData] {
        ...
        for (idx, flight) in flights.enumerated() {
            ...
            print(" AeroDataBox (\(searchDate)) [\(idx)] DEP \(depICAO): STD=\(depScheduled ?? "nil") OUT=\(depRevised ?? "nil") T/O=\(depRunway ?? "nil")")
            ...
            print(" AeroDataBox (\(searchDate)) [\(idx)] ARR \(arrICAO): STA=\(arrScheduled ?? "nil") IN=\(arrRevised ?? "nil") LDG=\(arrRunway ?? "nil")")
            ...
            print(" AeroDataBox (\(searchDate)) [\(idx)] \(depICAO)\(arrICAO): OUT=\(depTime)(actual=\(departureIsActual)) IN=\(arrTime)(actual=\(arrivalIsActual)) STD=\(scheduledDepTime ?? "nil") STA=\(scheduledArrTime ?? "nil")")
            print(" AeroDataBox (\(searchDate)) [\(idx)] T/O=\(runwayDepTime ?? "nil") LDG=\(runwayArrTime ?? "nil") date=\(flightDate)")
