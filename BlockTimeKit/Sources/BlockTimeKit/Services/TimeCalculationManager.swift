...
public class TimeCalculationManager: @unchecked Sendable {
    ...
    public func calculateNightTime(
        ...
        guard let flightTimeHours = timeStringToHours(blockTime) else {
            print("DEBUG: TimeCalculationManager.calculateNightTime failed to convert blockTime '\(blockTime)' to hours")
            return ""
        }

        let parsedFlightDate = Self.flightDateFormatter.date(from: flightDate) ?? Date()

        print("DEBUG: Calling calculateNightTime with from=\(fromAirport), to=\(toAirport), departureUTC=\(departureUTC), flightTimeHours=\(flightTimeHours), flightDate=\(flightDate) -> \(parsedFlightDate)")
        ...
        if let nightHours = nightCalcService.calculateNightTime(
            ...
            print("DEBUG: calculateNightTime returned \(nightHours) hours")
...
extension TimeCalculationManager {
    ...
    public func recalculateTimes(
        ...
        if let context = buildCalculationContext(
            ...
            print(" recalculateTimes: Using CACHED context for night time calculation")
            ...
            print("recalculateTimes: Context unavailable, using ORIGINAL calculation")
