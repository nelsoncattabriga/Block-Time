...
public class FRMSCalculationService: @unchecked Sendable {
    ...
    private func getBaseLimitsA380A330B787(crewComplement: CrewComplement,
        ...
        guard let dutyLimit = getLHDutyLimit(crewComplement: crewComplement, restFacility: restFacility, limitType: limitType) else {
            ...
            print("FRMS: Failed to find LH duty limit for \(crewComplement.description) with \(restFacility.description)")
        ...
        guard let resolvedDuty = dutyLimit.dutyPeriodLimitPlanned ?? dutyLimit.dutyPeriodLimitDiscretion else {
            print("FRMS: No duty limit in LH_Operational_FltDuty for \(crewComplement.description)")
    ...
    private func calculateMinimumRestA380A330B787(afterDuty duty: FRMSDuty, limitType: FRMSLimitType) -> Double {
        ...
        guard let restReq = getLHRestRequirement(crewComplement: duty.crewComplement,
            ...
            // This should never happen if LH_Operational_FltDuty is complete
            print("FRMS: Failed to find LH rest requirement for \(duty.crewComplement.description) after \(String(format: "%.1f", duty.dutyTime))h duty")
        ...
        print("FRMS: Using default 12h rest for unhandled case: \(restReq.dutyPeriodThreshold)")
        return 12.0
    }
    ...
    public func createDuty(from flightSector: FlightSector,
        ...
        guard Self.cachedDateFormatter.date(from: flightSector.date) != nil else {
            ...
            if flightSector.date.contains("2023") || flightSector.date.contains("2024") || flightSector.date.contains("2025") {
                print("FRMS: Skipping flight on \(flightSector.date) - invalid date format")
            }
            return nil
        }
        ...
        if !flightSector.outTime.isEmpty && !flightSector.inTime.isEmpty {
            ...
            guard var outDate = Self.cachedTimeFormatter.date(from: "\(flightSector.date) \(outTimeStr)"),
                ...
                // Only log if in recent years (2023-2025)
                if flightSector.date.contains("2023") || flightSector.date.contains("2024") || flightSector.date.contains("2025") {
                    print("FRMS: Skipping flight on \(flightSector.date) - can't parse OUT(\(flightSector.outTime))/IN(\(flightSector.inTime)) times")
                }
                return nil
            }
