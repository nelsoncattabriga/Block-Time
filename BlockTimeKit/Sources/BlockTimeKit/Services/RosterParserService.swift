    ...
    private static func extractFlights(from lines: [String], pilotInfo: PilotInfo) -> [ParsedFlight] {
        ...
        for (index, line) in lines.enumerated() {
            ...
            if line.contains("1-CPT") {
                currentRole = "Captain"
                                print(" Role detected: Captain (line \(index))")
            } else if line.contains("1-F/O") {
                currentRole = "First Officer"
                                print(" Role detected: First Officer (line \(index))")
            }
            ...
            if let match = line.range(of: pattern, options: .regularExpression) {
                print("  Line \(index): \(line.trimmingCharacters(in: .whitespaces))")
                ...
                print("     Aircraft: \(equipmentCode)")
                print("     Positioning: \(isPositioning)")
